...
Thank you for contributing to DeepAgent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
Bundled skills (in `skills/`) ship with every DeepAgent install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `deepagent skills browse` (labeled "official") and install it with `deepagent skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `deepagent skills install`.
...
```bash
mkdir -p ~/.deepagent/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.deepagent/config.yaml
touch ~/.deepagent/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=***' >> ~/.deepagent/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/deepagent" ~/.local/bin/deepagent

# Verify
deepagent doctor
deepagent chat -q "Hello"
```
...
│
├── skills/                   # Bundled skills (copied to ~/.deepagent/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (deepagent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.deepagent/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.deepagent/`)

| Path | Purpose |
|------|---------|
| `~/.deepagent/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.deepagent/.env` | API keys and secrets |
| `~/.deepagent/auth.json` | OAuth credentials (Nous Portal) |
| `~/.deepagent/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.deepagent/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.deepagent/state.db` | SQLite session database |
| `~/.deepagent/sessions/` | JSON session logs |
| `~/.deepagent/cron/` | Scheduled job data |
| `~/.deepagent/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. JSON logs go to `~/.deepagent/sessions/`.
...
The user may skip setup and keep loading the skill. DeepAgent only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `deepagent setup` or update `~/.deepagent/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing DeepAgent tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `deepagent --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

DeepAgent uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.deepagent/skins/<name>.yaml`:
...
DeepAgent runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
DeepAgent has terminal access. Security matters.
...
2. **Test manually**: Run `deepagent` and exercise the code path you changed
