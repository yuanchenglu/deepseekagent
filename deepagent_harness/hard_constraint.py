...
# B7 修复：必须类/禁止类的匹配必须在遇到另一类触发词时提前截断，
# 否则"必须X，禁止Y"会被当成一个长 requirement，把"禁止Y"也吃进关键词。
# 用非捕获组在字符类中加入对另一类触发词的阻断。
_PROHIBITION_STOP = r"(?:不要|不可以|不能|禁止|严禁|不准|不得|千万别|绝不要|绝对不要|必须|一定要|务必|需要|确保|请一定|千万要|应当|应该)"
_REQUIREMENT_STOP = r"(?:必须|一定要|务必|需要|确保|请一定|千万要|应当|应该|不要|不可以|不能|禁止|严禁|不准|不得|千万别|绝不要|绝对不要)"

_PROHIBITION_PATTERNS = [
    # 80 字符内遇到句末标点 或 反向触发词时截断
    re.compile(r"(?:不要|不可以|不能|禁止|严禁|不准|不得|千万别|绝不要|绝对不要)\s*((?:(?!" + _PROHIBITION_STOP + r")[^。！？\n]){2,80})"),
    # "别/勿" 开头的短语（更短：1-79 字符）
    re.compile(r"(?:别|勿)\s*([^\s，。！？\n](?:(?!" + _PROHIBITION_STOP + r")[^。！？\n]){1,79})"),
]

# 必须类触发词：必须/一定要/务必/需要/确保/请一定/千万要/应当/应该
_REQUIREMENT_PATTERNS = [
    re.compile(r"(?:必须|一定要|务必|需要|确保|请一定|千万要|应当|应该)\s*((?:(?!" + _REQUIREMENT_STOP + r")[^。！？\n]){2,80})"),
]

# 误匹配过滤模式：这些情况虽然触发了关键词但不是约束
_FALSE_POSITIVE_PATTERNS = [
    # "我不需要帮助"/"我需要你帮我..." 等表述个人状态/求助的用法，不是对 Agent 的约束
    # B4 修复：加上"我需要"——这是"我需要你帮我做 X"求助句，不是"你必须做到 X"约束
    re.compile(r"我不需要|我不要|别管我|不要问我|不用你|我不想|我需要"),
    # "你能不能？""要不要？""要不要用 X？""是否需要..."等疑问句，不是指令
    # B4 修复：去掉 ^ 锚点——疑问句可能出现在文本中部也要过滤
    re.compile(r"(?:你)?(?:能不?能|要不要|可不可以|是不是|是否需要|需要吗)[?？]?"),
    # 引用/代码块中的"不要"（如文档中举例）
...
class HardConstraintExtractor:
    ...
    def _is_false_positive(self, text: str, original_text: str = "", match_start: int = 0) -> bool:
        ...
        # B4 修复：疑问句中 "要不要/会不会/能不能不..." 是选择问句，不是约束
        # 如 "要不要用 Python 写？" → 匹配到 "不要用 Python 写" 是误判
        if original_text and match_start > 0:
            # 看触发词前 2 字符是否有 "要/会/能/可以"
            trigger_prefix = original_text[max(0, match_start - 2):match_start]
            if trigger_prefix and trigger_prefix[-1] in "要会能":
                return True

        # B4 修复：检查触发词在原始文本中的上下文，处理"我需要你帮我"之类
        # 因为传入的 text 是匹配到的片段（如"需要你帮我写个函数"），需要看原文位置
        if match_start > 0 and original_text:
            # 检查触发词前 1-3 个字符是否是"我"
            prefix = original_text[max(0, match_start - 3):match_start]
            if "我" in prefix:
                return True
            # "要不要"/"是否需要" 在疑问句中
            lookback = original_text[max(0, match_start - 5):match_start]
            if any(kw in lookback for kw in ["要不", "是否", "难道", "请问"]):
                return True
