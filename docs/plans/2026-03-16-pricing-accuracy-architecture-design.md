...
DeepAgent should only show dollar costs when they are backed by an official source for the user's actual billing path.
...
Current DeepAgent behavior has four structural issues:
...
DeepAgent must stop keying pricing solely by `model`.
...
- manual `deepagent pricing sync`

## Reconciliation Architecture

Live requests may produce only an estimate initially. DeepAgent should reconcile them later when a provider exposes actual billed cost.

Suggested flow:

1. Agent call completes.
2. DeepAgent stores canonical usage plus reconciliation ids.
3. DeepAgent computes an immediate estimate if a pricing source exists.
...
That path cannot satisfy the product requirement. DeepAgent should instead migrate to:
...
This is the minimum architecture that makes the statement "DeepAgent pricing is backed by official sources where possible, and otherwise clearly labeled" defensible.
