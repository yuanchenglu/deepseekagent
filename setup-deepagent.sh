...
echo ""
echo -e "${CYAN}⚕ DeepAgent Setup${NC}"
echo ""
...
echo -e "${CYAN}→${NC} Setting up deepagent command..."

HERMES_BIN="$SCRIPT_DIR/venv/bin/deepagent"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/deepagent"
echo -e "${GREEN}✓${NC} Symlinked deepagent → $COMMAND_LINK_DISPLAY_DIR/deepagent"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# DeepAgent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
HERMES_SKILLS_DIR="${DEEPAGENT_HOME:-${HERMES_HOME:-$HOME/.deepagent}}/skills"
mkdir -p "$HERMES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.deepagent/skills/ ..."
...
if is_termux; then
    ...
    echo "  deepagent gateway install # Install gateway service (messaging + cron)"
fi
echo "  deepagent cron list     # View scheduled jobs"
echo "  deepagent doctor        # Diagnose issues"
echo ""
