...
_HERMES_ENV_PATH = (
    r'(?:~\/\.(?:hermes|deepagent)/|'
    r'(?:\$home|\$\{home\})/\.(?:hermes|deepagent)/|'
    r'(?:\$(?:hermes|deepagent)_home|\$\{(?:hermes|deepagent)_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\b(?:hermes|deepagent)\s+gateway\s+(stop|restart)\b', "stop/restart hermes/deepagent gateway (kills running agents)"),
    (r'\b(?:hermes|deepagent)\s+update\b', "hermes/deepagent update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart hermes-gateway' or 'deepagent gateway restart')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart hermes-gateway' or 'deepagent gateway restart')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(hermes|deepagent|gateway|cli\.py)\b', "kill hermes/deepagent/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill hermes`/`pkill deepagent` but not
