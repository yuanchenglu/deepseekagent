...
Backend is selected during ``deepagent tools`` setup (web.backend in config.yaml).
When available, DeepAgent can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``deepagent tools``).
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_nous_tools_enabled():
        message += (
            ...
            "(`deepagent model`) and provide FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN."
        )
...
def _get_firecrawl_client():
    ...
    DeepAgent falls back to the Firecrawl tool-gateway for logged-in Nous Subscribers.
    """
