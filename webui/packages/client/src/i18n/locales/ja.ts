export default {
  ...
  runtimeVersions: {
    ...
    useVersion: '次回起動時に使用',
    deleteVersion: '削除',
    deleteRuntimeConfirm: 'DeepAgent Runtime {version} を削除しますか？',
    ...
    activateSuccess: 'バージョン選択を保存しました。DeepAgent を再起動すると反映されます。',
    jobStatus: {
      queued: '待機中',
      running: '実行中',
      completed: '完了',
  ...
  chat: {
    ...
    contextEditHint: '一般的な値：256k (DeepAgent 既定), 128k (GPT-4), 32k (GPT-3.5)',
    contextEditSave: '保存',
    ...
    emptyState: 'DeepAgent Runtime と会話を開始しましょう',
    ...
    openHistory: '履歴を開く',
    hermesHistory: 'DeepAgent 履歴',
    historyScopeHint: '現在の profile の DeepAgent 履歴セッションをソース別に読み取り専用で表示します。',
    ...
    searchScope: '検索範囲: Web UI のローカルセッション DB のみ。読み取り専用の DeepAgent 履歴セッションは含まれません。',
  ...
  skills: {
    ...
    writeApprovalDescription: 'DeepAgent Runtime が保留したメモリとスキルの書き込みを承認または拒否します。',
    writeApprovalRefresh: '更新',
    writeApprovalEmpty: '承認待ちの書き込みはありません。',
    writeApprovalUnsupported: 'この DeepAgent Runtime バージョンは書き込み承認に対応していません。この機能を使うには DeepAgent Runtime を更新してください。',
  ...
  plugins: {
    title: 'プラグイン',
    refresh: '更新',
    notice: '検出可能な DeepAgent プラグイン manifest の読み取り専用インベントリです。検出メタデータはプラグインコードを読み込まずに取得します。v1 の管理操作は CLI のままで、変更は新しい DeepAgent セッションで有効になります。',
  ...
  models: {
    ...
    copilotAddDetected: 'このマシンで GitHub Copilot OAuth トークンを検出しました。「追加」をクリックして DeepAgent で Copilot を有効化します。',
    copilotAddSourceEnv: 'ソース: ~/.deepagent/config/.env (COPILOT_GITHUB_TOKEN)',
    copilotAddSourceGhCli: 'ソース: gh CLI (gh auth token)',
    copilotAddSourceAppsJson: 'ソース: VS Code Copilot 拡張機能 (apps.json)',
    copilotDeleteHintEnv: 'この操作で ~/.deepagent/config/.env の COPILOT_GITHUB_TOKEN を消去します。他のツールには影響しません。',
    copilotDeleteHintGhCli: 'Copilot は DeepAgent 上で非表示になります。gh CLI のログインには影響しません — `gh auth status` は引き続きログイン状態を表示します。',
    copilotDeleteHintAppsJson: 'Copilot は DeepAgent 上で非表示になります。VS Code Copilot 拡張機能のログインには影響しません。',
    customBadge: 'カスタム',
    previewBadge: 'プレビュー',
    disabledBadge: '利用不可',
    ...
    visibilityHint: 'Web UI のモデル選択とモデルページの表示だけに影響します。DeepAgent CLI の provider/model 設定は変更されず、実際の呼び出しは元のモデル ID を使います。',
    ...
    aliasManage: '表示名',
    aliasHint: 'Web UI の表示名だけを変更します。DeepAgent には元のモデル ID が送信されます。',
    aliasEdit: '名前変更',
  ...
  profiles: {
    ...
    rename: '名前変更',
    delete: '削除',
    switchTo: 'DeepAgent Profile を切り替え',
    switchConfirm: '`hermes profile use {name}` を実行し、DeepAgent CLI の active profile を切り替えます。続行しますか？',
    switchSuccess: 'DeepAgent active profile を「{name}」に切り替えました',
    switchFailed: 'DeepAgent Profile の切り替えに失敗しました。ゲートウェイの手動再起動が必要な場合があります。',
  ...
  settings: {
    ...
    gatewayAutoStart: {
      title: 'Gateway 自動起動',
      description: 'Web UI 起動時に自動起動するローカル DeepAgent profile gateway を制御します。この設定は Web UI 設定に保存され、profile config.yaml には保存されません。',
      enabled: 'Gateway を自動起動',
      enabledHint: '無効にすると、起動時に profile gateway を自動起動しません。',
      management: '統一 gateway',
      managementHint: 'DeepAgent Runtime 0.17.0 以降が必要です。有効にすると Web UI が multiplex_profiles を書き込み、default gateway がすべての profiles を処理します。他の profile では Feishu、Weixin、QQBot などのメッセージング入口を個別設定できません。無効にするとその設定を削除します。',
      managementAuto: '自動',
    ...
    session: {
      ...
      memoryWriteApprovalHint: 'DeepAgent Runtime のメモリ書き込みを保存前に承認待ちにします。',
      skillsWriteApproval: 'スキル書き込みを確認',
      skillsWriteApprovalHint: 'DeepAgent Runtime のスキル変更を保存前に承認待ちにします。',
      writeApprovalTitle: '書き込み承認待ち',
      writeApprovalDescription: 'DeepAgent Runtime が保留したメモリとスキルの書き込みを承認または拒否します。',
      writeApprovalRefresh: '更新',
      writeApprovalEmpty: '承認待ちの書き込みはありません。',
  ...
  codingAgents: {
    ...
    codexDescription: "リポジトリ作業向けの OpenAI CLI と DeepAgent openai-codex プロバイダーフローです。",
    copyCommand: "コピー",
    commandCopied: "コマンドをコピーしました",
    ...
    installFailedDeepAgentHint: "インストールに失敗しました。DeepAgent にインストールを依頼してください。",
    ...
    notes: {
      ...
      codexAuth: "DeepAgent 管理の OpenAI Codex OAuth 認証情報を追加します。",
  ...
  skillsUsage: {
    title: 'スキル使用状況',
    subtitle: 'DeepAgent セッションでのスキル読み込みと編集を追跡します',
    refresh: '更新',
  ...
  changelog: {
    ...
    new_0_6_22_5: 'DeepAgent チャットに /learn コマンドを追加し、Agent Bridge 実行は選択した workspace を cwd として使います。次の DeepAgent Runtime リリース待ちなのは、デスクトップ版の /learn と MoA コマンド機能だけです',
    ...
    new_0_6_18_2: 'DeepAgent MCP を api、devices、use の toolset に分割し、session、model、chat-run、ops 操作を追加しました',
    ...
    new_0_6_18_6: 'Desktop と runtime setup は、Unicode-safe な Windows PATH 登録、DeepAgent Runtime 0.17.0 runtime publish 対応、mobile PWA metadata、profile/auth recovery 修正で安全になりました',
    ...
    new_0_6_13_5: 'デスクトップ更新の信頼性を改善しました。Windows アップグレード時に残留 DeepAgent プロセスを閉じ、Cloudflare を先に確認し、GitHub をフォールバック更新元として保持します',
