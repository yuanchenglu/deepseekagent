export default {
  ...
  runtimeVersions: {
    ...
    useVersion: '下次啟動使用',
    deleteVersion: '刪除',
    deleteRuntimeConfirm: '刪除 DeepAgent Runtime {version}？',
    ...
    activateSuccess: '版本選擇已儲存，重啟 DeepAgent 後生效。',
    jobStatus: {
      queued: '排隊中',
      running: '下載中',
      completed: '已完成',
  ...
  chat: {
    ...
    openHistory: '開啟歷史',
    hermesHistory: 'DeepAgent 歷史',
    historyScopeHint: '這裡按來源以唯讀方式查看目前 profile 的 DeepAgent 歷史工作階段。',
    noSessions: '目前無工作階段',
    searchTitle: '搜尋工作階段',
    searchSubtitle: '依標題或訊息內容搜尋',
    searchScope: '搜尋範圍：僅 Web UI 本地工作階段資料庫；不包含唯讀 DeepAgent 歷史工作階段。',
  ...
  skills: {
    ...
    writeApprovalDescription: '批准或拒絕 DeepAgent Runtime 暫存的記憶和技能寫入。',
    writeApprovalRefresh: '重新整理',
    writeApprovalEmpty: '暫無待審核寫入。',
    writeApprovalUnsupported: '目前 DeepAgent Runtime 版本不支援寫入審核。請升級 DeepAgent Runtime 後再使用。',
  ...
  plugins: {
    title: '插件',
    refresh: '重新整理',
    notice: '唯讀顯示可發現的 DeepAgent 插件 manifest。發現元資料讀取不會載入插件程式碼。v1 管理動作仍保留在 CLI，新 DeepAgent 工作階段生效。',
    loadFailed: '載入插件失敗',
    commandCopied: '指令已複製',
  ...
  petdex: {
    title: 'Petdex 寵物',
    subtitle: '瀏覽 DeepAgent 桌寵使用的 petdex 公共圖庫。',
    refresh: '重新整理',
    adopt: '領取',
    active: '使用中',
    adopted: '已領取 {name}',
  ...
  models: {
    ...
    copilotAddDetected: '已在本機偵測到 GitHub Copilot OAuth 憑證，點擊「新增」即可在 DeepAgent 中啟用 Copilot。',
    copilotAddSourceEnv: '來源：~/.deepagent/config/.env（COPILOT_GITHUB_TOKEN）',
    copilotAddSourceGhCli: '來源：gh CLI（gh auth token）',
    copilotAddSourceAppsJson: '來源：VS Code Copilot 插件（apps.json）',
    copilotDeleteHintEnv: '此操作會清除 ~/.deepagent/config/.env 中的 COPILOT_GITHUB_TOKEN，不影響其他工具。',
    copilotDeleteHintGhCli: 'Copilot 將從 DeepAgent 清單移除。不會影響 gh CLI —— `gh auth status` 仍顯示已登入。',
    copilotDeleteHintAppsJson: 'Copilot 將從 DeepAgent 清單移除。不會影響 VS Code Copilot 插件的登入。',
    customBadge: '自訂',
    previewBadge: '預覽',
    disabledBadge: '不可用',
    ...
    visibilityHint: '僅影響 Web UI 的模型選擇器和模型頁展示，不會改寫 DeepAgent CLI 的 provider/model 配置。實際呼叫仍使用原始模型 ID。',
    ...
    aliasHint: '僅修改 Web UI 顯示名，傳送給 DeepAgent 的仍是原始模型 ID。',
  ...
  profiles: {
    ...
    rename: '重新命名',
    delete: '刪除',
    switchTo: '切換 DeepAgent Profile',
    switchConfirm: '將執行 `hermes profile use {name}` 並切換 DeepAgent CLI 的 active profile，是否繼續？',
    switchSuccess: 'DeepAgent active profile 已切換為「{name}」',
    switchFailed: '切換 DeepAgent Profile 失敗，閘道可能需要手動重新啟動',
  ...
  settings: {
    ...
    gatewayAutoStart: {
      title: 'Gateway 自動啟動',
      description: '控制 Web UI 啟動時自動拉起哪些本機 DeepAgent profile gateway。此項寫入 Web UI 設定，不寫入 profile config.yaml。',
      enabled: '自動啟動 gateway',
      enabledHint: '關閉後，啟動期不會自動拉起任何 profile gateway。',
      management: '統一 gateway',
      managementHint: '需要 DeepAgent Runtime 0.17.0 及以上版本。開啟後會寫入 multiplex_profiles，讓 default gateway 服務所有 profiles；其他 profile 不能單獨設定飛書、微信、QQBot 等通訊工具入口。關閉後會移除此設定。',
      managementAuto: '自動',
    ...
    session: {
      ...
      memoryWriteApprovalHint: 'DeepAgent Runtime 寫入 MEMORY.md / USER.md 前先進入待審核佇列。',
      skillsWriteApproval: '技能寫入審核',
      skillsWriteApprovalHint: 'DeepAgent Runtime 建立或修改技能前先進入待審核佇列。',
      writeApprovalTitle: '待審核寫入',
      writeApprovalDescription: '批准或拒絕 DeepAgent Runtime 暫存的記憶和技能寫入。',
      writeApprovalRefresh: '重新整理',
      writeApprovalEmpty: '暫無待審核寫入。',
  ...
  codingAgents: {
    ...
    codexDescription: "OpenAI CLI，以及 DeepAgent openai-codex 提供商的倉庫任務流程。",
    copyCommand: "複製",
    commandCopied: "命令已複製",
    ...
    installSuccess: "安裝完成",
    installFailed: "安裝失敗",
    installFailedDeepAgentHint: "安裝失敗，請讓 DeepAgent 幫你安裝。",
    ...
    notes: {
      ...
      codexAuth: "新增 DeepAgent 管理的 OpenAI Codex OAuth 憑證。",
  ...
  skillsUsage: {
    title: '技能用量',
    subtitle: '追蹤 DeepAgent 工作階段中的技能載入與編輯',
    refresh: '重新整理',
  ...
  changelog: {
    ...
    new_0_6_22_5: 'DeepAgent 聊天新增 /learn 命令，Agent Bridge 執行會使用所選 workspace 作為 cwd；由於 DeepAgent Runtime 最新版本尚未發布，只有桌面端的 /learn 和 MoA 指令能力需要等新版 DeepAgent Runtime 發布後可用',
    ...
    new_0_6_21_3: 'DeepAgent MCP 的 use 工具集新增工作流操作：列表、詳情、建立、更新、刪除、執行、停止、刪除執行記錄，以及從指定節點重跑',
    ...
    new_0_6_21_7: '單聊、群聊和工作流頂部列對齊更一致；小方盒頁面改為售罄狀態；BSL 授權範圍更清楚；補充 /learn 接入規劃，並移除廢棄 DeepAgent proxy 程式碼',
    ...
    new_0_6_18_2: 'DeepAgent MCP 拆分為 api、devices 和 use 工具集，新增 session、model、chat-run 與 ops 相關操作',
    ...
    new_0_6_18_6: '桌面與 runtime 設定更安全：Windows PATH 註冊支援 Unicode，支援發布 DeepAgent Runtime 0.17.0 runtime，並補齊行動端 PWA metadata 和多項 profile/auth 恢復修復',
    ...
    new_0_6_13_5: '桌面更新更可靠：Windows 升級會關閉殘留 DeepAgent 行程，優先檢查 Cloudflare，並保留 GitHub 更新源兜底',
