export default {
  ...
  runtimeVersions: {
    ...
    useVersion: '下次启动使用',
    deleteVersion: '删除',
    deleteRuntimeConfirm: '删除 DeepAgent Runtime {version}？',
    ...
    activateSuccess: '版本选择已保存，重启 DeepAgent 后生效。',
    jobStatus: {
      queued: '排队中',
      running: '下载中',
      completed: '已完成',
  ...
  chat: {
    ...
    openHistory: '打开历史',
    hermesHistory: 'DeepAgent 历史',
    historyScopeHint: '这里按来源只读查看当前 profile 的 DeepAgent 历史会话。',
    noSessions: '暂无会话',
    searchTitle: '搜索会话',
    searchSubtitle: '按标题或消息内容搜索',
    searchScope: '搜索范围：仅 Web UI 本地会话库；不包含只读 DeepAgent 历史会话。',
  ...
  skills: {
    ...
    writeApprovalDescription: '批准或拒绝 DeepAgent Runtime 暂存的记忆和技能写入。',
    writeApprovalRefresh: '刷新',
    writeApprovalEmpty: '暂无待审核写入。',
    writeApprovalUnsupported: '当前 DeepAgent Runtime 版本不支持写入审核。请升级 DeepAgent Runtime 后再使用。',
  ...
  plugins: {
    title: '插件',
    refresh: '刷新',
    notice: '只读展示可发现的 DeepAgent 插件 manifest。发现元数据读取不会加载插件代码。v1 管理动作仍保留在 CLI，新 DeepAgent 会话生效。',
    loadFailed: '加载插件失败',
    commandCopied: '命令已复制',
  ...
  petdex: {
    title: 'Petdex 宠物',
    subtitle: '浏览 DeepAgent 桌宠使用的 petdex 公共图库。',
    refresh: '刷新',
    adopt: '领取',
    active: '使用中',
    adopted: '已领取 {name}',
  ...
  models: {
    ...
    copilotAddDetected: '已在本机检测到 GitHub Copilot OAuth 凭证，点击「添加」即可在 DeepAgent 中启用 Copilot。',
    copilotAddSourceEnv: '来源：~/.deepagent/config/.env（COPILOT_GITHUB_TOKEN）',
    copilotAddSourceGhCli: '来源：gh CLI（gh auth token）',
    copilotAddSourceAppsJson: '来源：VS Code Copilot 插件（apps.json）',
    copilotDeleteHintEnv: '此操作会清除 ~/.deepagent/config/.env 中的 COPILOT_GITHUB_TOKEN，不影响其他工具。',
    copilotDeleteHintGhCli: 'Copilot 将从 DeepAgent 列表移除。不会影响 gh CLI —— `gh auth status` 仍显示已登录。',
    copilotDeleteHintAppsJson: 'Copilot 将从 DeepAgent 列表移除。不会影响 VS Code Copilot 插件的登录。',
    ...
    aliasHint: '仅修改 Web UI 显示名，发送给 DeepAgent 的仍是原始模型 ID。',
    ...
    visibilityHint: '仅影响 Web UI 的模型选择器和模型页展示，不会改写 DeepAgent CLI 的 provider/model 配置。实际调用仍使用原始模型 ID。',
  ...
  profiles: {
    ...
    delete: '删除',
    editConfig: '编辑配置文件',
    switchTo: '切换 DeepAgent Profile',
    switchConfirm: '将执行 `hermes profile use {name}` 并切换 DeepAgent CLI 的 active profile，是否继续？',
    switchSuccess: 'DeepAgent active profile 已切换为 "{name}"',
    switchFailed: '切换 DeepAgent Profile 失败，网关可能需要手动重启',
  ...
  settings: {
    ...
    gatewayAutoStart: {
      title: 'Gateway 自动启动',
      description: '控制 Web UI 启动时自动拉起哪些本地 DeepAgent profile gateway。该项写入 Web UI 配置，不写入 profile config.yaml。',
      enabled: '自动启动 gateway',
      enabledHint: '关闭后，启动期不会自动拉起任何 profile gateway。',
      management: '统一 gateway',
      managementHint: '需要 DeepAgent Runtime 0.17.0 及以上版本。开启后会写入 multiplex_profiles，让 default gateway 服务所有 profiles；其他 profile 不能单独配置飞书、微信、QQBot 等通讯工具入口。关闭后会移除该配置。',
      managementAuto: '自动',
    ...
    session: {
      ...
      memoryWriteApprovalHint: 'DeepAgent Runtime 写入 MEMORY.md / USER.md 前先进入待审核队列。',
      skillsWriteApproval: '技能写入审核',
      skillsWriteApprovalHint: 'DeepAgent Runtime 创建或修改技能前先进入待审核队列。',
      writeApprovalTitle: '待审核写入',
      writeApprovalDescription: '批准或拒绝 DeepAgent Runtime 暂存的记忆和技能写入。',
      writeApprovalRefresh: '刷新',
      writeApprovalEmpty: '暂无待审核写入。',
  ...
  codingAgents: {
    ...
    codexDescription: "OpenAI CLI，以及 DeepAgent openai-codex provider 的仓库任务流程。",
    copyCommand: "复制",
    commandCopied: "命令已复制",
    ...
    installSuccess: "安装完成",
    installFailed: "安装失败",
    installFailedDeepAgentHint: "安装失败，请让 DeepAgent 帮你安装。",
    ...
    notes: {
      ...
      codexAuth: "添加 DeepAgent 管理的 OpenAI Codex OAuth 凭证。",
  ...
  skillsUsage: {
    title: '技能用量',
    subtitle: '跟踪 DeepAgent 会话中的技能加载和编辑',
    refresh: '刷新',
  ...
  changelog: {
    ...
    new_0_6_22_5: 'DeepAgent 聊天新增 /learn 命令，Agent Bridge 运行会使用所选 workspace 作为 cwd；由于 DeepAgent Runtime 最新版本尚未发布，只有桌面端的 /learn 和 MoA 指令能力需要等新版 DeepAgent Runtime 发布后可用',
    ...
    new_0_6_21_3: 'DeepAgent MCP 的 use 工具集新增工作流操作：列表、详情、创建、更新、删除、运行、停止、删除执行记录，以及从指定节点重跑',
    ...
    new_0_6_21_7: '单聊、群聊和工作流顶部栏对齐更一致；小方盒页面改为售罄状态；BSL 授权范围更清晰；补充 /learn 接入规划，并移除废弃 DeepAgent proxy 代码',
    ...
    new_0_6_18_2: 'DeepAgent MCP 拆分为 api、devices 和 use 工具集，新增 session、model、chat-run 与 ops 相关操作',
    ...
    new_0_6_18_6: '桌面与 runtime 设置更安全：Windows PATH 注册支持 Unicode，支持发布 DeepAgent Runtime 0.17.0 runtime，并补齐移动端 PWA 元数据和多项 profile/auth 恢复修复',
    ...
    new_0_6_13_5: '桌面更新更可靠：Windows 升级会关闭残留 DeepAgent 进程，优先检查 Cloudflare，并保留 GitHub 更新源兜底',
